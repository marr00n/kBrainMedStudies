...
## Phase 2 — Atomise into Concept Notes
- Go through highlights and identify distinct concepts
- Create a separate markdown note for each concept
- Title each note as a **full one-sentence claim**, not a label
  - ❌ "Baroreceptor reflex"
  - ✅ "The baroreceptor reflex buffers acute BP changes via vagal and sympathetic modulation of HR and vascular tone"
  - Test: can you recall what the note is about from the title alone, months later?
- Each note contains only content supporting that single concept
- Rewrite and annotate highlights in your own words beneath the title
- If a highlight contains two ideas → split across two notes
- If multiple highlights support the same concept → merge into one note
- Heuristic: if you want a second heading inside a note, it needs splitting
...
- Do not skip Phase 2 and jump from highlights straight to canvas
- Connecting long, unprocessed notes on canvas ≈ putting them in the same folder
- Atomisation is what makes spatial mapping meaningful