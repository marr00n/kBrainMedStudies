...
var removeBlankLinesPreserveStructure = function(text, fixHyphenation, fixWhitespace, preserveListSpacing) {
  if (fixHyphenation) {
    text = text.replace(/-(\r\n|\r|\n)/g, "");
  }
  ...
  const isHorizontalRule = function(line) {
    return line.trim() === "---";
  };
  ...
  const ensureBlankLine = function() {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isHorizontalRule(line)) {
      if (previousWasList && preserveListSpacing) {
        ensureBlankLine();
      } else if (!previousWasList) {
        ensureBlankLine();
      }
      output.push("---");
      output.push("");
...
var RemoveNewline = class extends import_obsidian2.Plugin {
  constructor() {
    ...
    this.removeBlankLines = (text) => {
      if (this.settings.preserveListSpacing) {
        return removeBlankLinesPreserveStructure(text, false, false, true);
      }
    ...
    this.removeBlankLinesPreserveStructureFromSelection = (editor) => {
      const transform = (text) => removeBlankLinesPreserveStructure(text, false, false, true);
  ...
  async onload() {
    ...
    this.addCommand({
      id: "remove-blank-lines-from-selection-preserve-structure",
      name: "Remove newlines: Remove blank lines from selection, preserve structure.",
      editorCheckCallback: (checking, editor, ctx) => {
        if (editor.somethingSelected()) {
          if (!checking) {
            this.removeBlankLinesPreserveStructureFromSelection(editor);
...
var RemoveNewlineSettingsTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian2.Setting(containerEl).setName("Preserve list spacing when removing blank lines").setDesc(
      "Keep a blank line after list blocks and around headings/horizontal rules to preserve structure."
    ).addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.preserveListSpacing).onChange(async (value) => {
        this.plugin.settings.preserveListSpacing = value;
        await this.plugin.saveSettings();
      })
    );
  }
